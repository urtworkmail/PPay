...
| Notifications | No email/SMS infrastructure — invite links and API keys are shown once in-app instead | Add transactional email/SMS (invite emails, receipts, dunning notices, payout confirmations) — currently deliberately deferred rather than half-built. |
...
| Rate limiting | None yet | Add per-API-key and per-IP rate limiting before any public-facing production traffic. |
...
| Monitoring & alerting | None beyond application logs | Uptime, error-rate, and fraud-signal monitoring/alerting before real money is at risk. |
| Disaster recovery | None | Backup/restore strategy and documented RTO/RPO for the database and any stored secrets. |
| Disputes/chargebacks | Explicitly not built — "Coming soon" in the UI, stated honestly rather than faked | New module: dispute lifecycle, evidence submission, chargeback liability handling with the acquirer. |
